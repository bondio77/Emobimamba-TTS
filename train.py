...
from training.reporter import Reporter
from contextlib import contextmanager
from torch.utils.data import DataLoader
from distutils.version import LooseVersion
from utils.model import get_model, get_parser
from torch.utils.tensorboard import SummaryWriter
from utils.torch_utils.recursive_op import recursive_average
from training.schedulers.abs_scheduler import AbsBatchStepScheduler
from utils.torch_utils.add_gradient_noise import add_gradient_noise
from utils.torch_utils.set_all_random_seed import set_all_random_seed
from training.average_nbest_models import average_nbest_models
...
def main_worker(gpu, args, configs):
    ...
    for iepoch in range(start_epoch, args.max_epoch + 1):
        ...
        torch.save({"model":model.state_dict()}, Path(join(output_dir, f"{iepoch}epoch.pth")))
...
def train_one_epoch(model, optimizers, schedulers, iterator, reporter, summary_writer, options, distributed, ngpu, epoch):
    ...
    model.intensity_model = RankModel(config).to('cuda:1')
    ...
    w = sum(p.sum().item() for p in model.intensity_model.intensity_extractor.parameters())
    # print('intensity0:', w)
...
def validate_one_epoch(model, iterator, reporter, options, distributed, ngpu):
    ...
    # [For distributed] Because iteration counts are not always equals between
    # processes, send stop-flag to the other processes if iterator is finished
    iterator_stop = torch.tensor(0).to("cuda:1" if ngpu > 0 else "cpu")
    for batch in iterator:
        ...
        batch = to_device(batch, "cuda:1" if ngpu > 0 else "cpu")
        if no_forward_run:
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--preprocess_config", type=str, default="./configs/esd/preprocess.yaml",
                        help="path to preprocess.yaml",)
    parser.add_argument("--model_config", type=str, default="./configs/esd/model_6L.yaml",
                        help="path to model.yaml")
    parser.add_argument("--option_config", type=str, default="./configs/esd/option.yaml",
                        help="path to option.yaml")
    parser.add_argument("--log_path", type=str, default="./model_checkpoint/context_EGCA_mamba1_8/log",
                        help="path to log")
    parser.add_argument("--result_path", type=str, default="./model_checkpoint/context_EGCA_mamba1_8/",
