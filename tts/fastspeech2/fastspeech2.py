...
class FastSpeech2(AbsTTS):
    ...
    def __init__(
        ...
        self.intensity_projector = torch.nn.Linear(256, 192).to('cuda:1')
        self.hs_projector = torch.nn.Linear(768, 384).to('cuda:1')
        ...
        #     "Surprise": 4
        # }
        # self.intensity_info = torch.load('/home/user/PycharmProjects/emotion_inensity/intensity_fs2/tts/fastspeech2/intensity_info_reduce_lambda_attn_emo.pt')

        # self.intensity_info = torch.load('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/tts/fastspeech2/enhance_ver/emotion_logit_og/intensity_info_enhance_re_og2.pt')
        # self.intensity_info = torch.load('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/intensity_train/runs/dual_EGCA2/intensity_info_EGCA3.pt')
        self.intensity_info = torch.load('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/intensity_train/runs/context_EGCA/intensity_info_EGCA3.pt')
        ...
        if spks is not None and spks > 1:
            self.spks = spks
            self.sid_emb = torch.nn.Embedding(spks, 192)
        self.langs = None
    ...
    def _forward(
        ...
        #
        if is_inference:
            w = sum(p.sum().item() for p in self.intensity_model.intensity_extractor.parameters())
            # print('intensity1:', w)
            ...
            # intensity_rep = self.intensity_projector(intensity_rep)
            ...
            for i, emotion in enumerate(emotions):
                emotion_id = emotion.item()
                if emotion_id == 0:  # Neutral
                    rep = torch.zeros(256).unsqueeze(0).to(hs.device)
                    phone_intensity_rep = rep.unsqueeze(1).expand(-1, hs.size(1), -1).squeeze(0)
                else:
                    _, rep = self.intensity_model.intensity_extractor(
                    ...
                    phone_intensity_rep = average_by_duration(rep[0], ds[i])
                intensity_rep.append(phone_intensity_rep)
            padded_intensity_rep = pad_sequences(intensity_rep)
            # intensity_rep = torch.stack(intensity_rep)
        # padded_intensity_rep = padded_intensity_rep.squeeze(1)
        reduce_intensity = self.intensity_projector(padded_intensity_rep)
        # LayerNorm & Scale 적용
        # regurarized_intensity_rep = self.intensity_scale * self.intensity_norm(intensity_rep)
        ...
        spk_it = torch.cat([reduce_intensity, sid_embs], dim=-1)
        ...
        # os.makedirs('./train_weired_improved_extractor', exist_ok=True)
        ...
        # np.save(f'./train_weired_improved_extractor/{emotions[0]}_{sids[0]}', valid_sample.detach().cpu().numpy())
        if self.use_cwt and self.use_classifier and self.use_pitch_max:
