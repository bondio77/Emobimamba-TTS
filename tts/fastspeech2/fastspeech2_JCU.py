...
class FastSpeech2(AbsTTS):
    ...
    def __init__(
        ...
        # print(torch.sum(self.intensity_model.intensity_extractor.transformer.layers[0].linear1.weight))
        # self.intensity_scale = torch.nn.Parameter(torch.ones(1)).to('cuda:0')
        # self.intensity_norm = torch.nn.LayerNorm(384).to('cuda:0')
        self.intensity_projector_frame = torch.nn.Linear(256, 192).to('cpu')
        self.intensity_projector_phone = torch.nn.Linear(256, 192).to('cpu')
        self.hs_projector = torch.nn.Linear(768, 384).to('cpu')
        ...
        #     "Surprise": 4
        # }
        # self.intensity_info = torch.load(
        #     '/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/tts/fastspeech2/intensity_info_attn_emo_re.pt')
        # self.intensity_info = torch.load('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/tts/fastspeech2/enhance_ver/consistency_1.0/intensity_info_enhance_JCU.pt')
        # self.intensity_info = torch.load('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/tts/fastspeech2/enhance_ver/emotion_logit_og/intensity_info_enhance_re_og.pt')
        # self.intensity_info = torch.load('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/intensity_train/runs/EGCA_consistency_re2/intensity_info_EGCA3.pt')
        self.intensity_info = torch.load('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/intensity_train/runs/EGCA/intensity_info_EGCA_23.pt')

        # self.intensity_proj = torch.nn.Sequential(
        #     torch.nn.Conv1d(256, 384, kernel_size=3, padding=1),
        #     torch.nn.BatchNorm1d(384),  # feature dimension에 대해 정규화
        #     torch.nn.GELU()
        # ).to('cuda:3')
        ...
        if spks is not None and spks > 1:
            self.spks = spks
            self.sid_emb = torch.nn.Embedding(spks, 192).to('cpu')
        self.langs = None
        if langs is not None and langs > 1:
            self.langs = langs
            self.lid_emb = torch.nn.Embedding(langs, adim)
        self.emos = None
        if emos is not None and emos > 1:
            self.emos = emos
            self.emo_emb = torch.nn.Embedding(emos, 192)
        ...
        self.feat_out = torch.nn.Linear(384, odim * reduction_factor)
        ...
        self.criterion = FastSpeech2Loss(
            use_masking=use_masking, use_weighted_masking=use_weighted_masking, use_cwt=self.use_cwt, use_classifier=self.use_classifier, use_pitch_max=self.use_pitch_max, use_equalizer=self.use_equalizer
        )
    ...
    def forward(
        ...
        energy: torch.Tensor,
        energy_frame: torch.Tensor,
        energy_lengths: torch.Tensor,
        energy_frame_lengths: torch.Tensor,
        ...
        # energy_frame = energy_frame[:, : energy_frame_lengths.max()]  # for data-parallel
        #
        batch_size = text.size(0)
        # target_len = max(feats_lengths.max(), pitch_per_frame_lengths.max())
        # if feats_lengths.max() < pitch_per_frame_lengths.max():
        #     feats = F.interpolate(feats.transpose(1, 2), size=target_len, mode='linear',
        #                                                                  align_corners=False).transpose(1, 2)
        #     # feats_frame = F.interpolate(feats_frame.transpose(1, 2), size=target_len, mode='linear',
        #     #                       align_corners=False).transpose(1, 2)
        # else:
        #     pitch_per_frame = F.interpolate(pitch_per_frame.unsqueeze(-1).transpose(1, 2), size=target_len, mode='linear',
        #                                                                  align_corners=False).transpose(1, 2)
        #     pitch_per_frame = pitch_per_frame.squeeze(-1)
        #
        #     energy_frame = F.interpolate(energy_frame.unsqueeze(-1).transpose(1, 2), size=target_len,
        #                                     mode='linear',
        #                                     align_corners=False).transpose(1, 2)
        #     energy_frame = energy_frame.squeeze(-1)
        ...
        ys,  ds, pps, pfs, pms, es, efs, epoch = feats, durations, pitch_per_phoneme.unsqueeze(2), pitch_per_frame, pitch_max_per_phoneme.unsqueeze(2), energy.unsqueeze(2), energy_frame.unsqueeze(2), epoch
        olens = feats_lengths #+ (target_len - feats_lengths.max())

        cs, cs_scale, cs_mean, cs_std = None, None, None,None
        ...
        if self.use_cwt and self.use_classifier and self.use_pitch_max:
            ...
            before_outs, after_outs, d_outs, p_outs, e_outs, p_m_outs, condition = self._forward(
                xs=xs,
                ilens=ilens,
                ys=ys,
                # ys_frame = ys_frame,
        ...
        if self.use_cwt and self.use_classifier and self.use_pitch_max:
            ...
            stats = dict(
                ...
                # duration_var_loss=duration_var_loss.item(),
        ...
        if not joint_training:
            ...
            return loss, stats, weight, after_outs, condition
        else:
            return loss, stats, after_outs, condition if after_outs is not None else before_outs
    ...
    def _forward(
        ...
        # ys_frame: Optional[torch.Tensor] = None,
        ...
        efs: Optional[torch.Tensor] = None,
        ...
        alpha: float = 1.0,
        epoch: int = 0,
        intensity_level: Optional[str] = None,  # 'min', 'medium', 'max'
        ...
        if self.use_equalizer:
            if is_inference:
                ...
                styles = self.style_equalizer(src_style, ref_style, hs)
            else:
                mel_masks = get_mask_from_lengths(olens, ys.shape[1], is_inference)
        ...
        if self.spks is not None:
            ...
            emo_embs = self.emo_emb(emotions.view(-1))
            sid_embs = sid_embs.unsqueeze(1)
            # hs = hs + sid_embs.unsqueeze(1) + emo_embs.unsqueeze(1)
        if self.langs is not None:
            lid_embs = self.lid_emb(lids.view(-1))
            hs = hs + lid_embs.unsqueeze(1)

        # intensity_input = torch.cat([pfs.unsqueeze(-1), efs, ys], dim=2)
        # pre_intensity_feature = self.intensity_model.base_model(intensity_input)
        #
        # intensity_rep = []
        # for i, emotion in enumerate(emotions):
        #     emotion_id = emotion.item()
        #     if emotion_id == 0:  # Neutral
        #         rep = torch.zeros(256).to(hs.device)
        #     else:
        #         # 해당 샘플의 pre_feature만 사용
        #         rep, _ = self.intensity_model.intensity_extractor(
        #             pre_intensity_feature[i:i + 1],  # [1, seq_len, dim]
        #             emotion.view(-1).unsqueeze(-1)
        #         )
        #     intensity_rep.append(rep.squeeze(0))
        # intensity_rep = torch.stack(intensity_rep)  # [B, 256]
        #
        # regurarized_intensity_rep = self.intensity_scale * self.intensity_norm(intensity_rep)
        #
        # # Expand & Concat
        # intensity_emb = regurarized_intensity_rep.unsqueeze(1)  # [B, 1, 256]
        # intensity_emb = intensity_emb.expand(-1, hs.size(1), -1)  # [B, T_text, 256]
        # hs = torch.cat([hs, intensity_emb], dim=-1)  # [B, T_text, adim+256]
        ...
        if is_inference:
            ...
            intensity_rep = intensity_rep.unsqueeze(0)
            padded_intensity_rep = intensity_rep.expand(-1, hs.size(1), -1)
            condition = None
            phone_intensity_rep = self.intensity_projector_phone(padded_intensity_rep)
            phone_intensity_reps = phone_intensity_rep
            # phone_intensity_reps = self.intensity_projector(padded_intensity_rep)
            ...
            for i, emotion in enumerate(emotions):
                emotion_id = emotion.item()
                if emotion_id == 0:  # Neutral
                    mean_rep = torch.zeros(256).unsqueeze(0).to(hs.device)
                    rep = torch.zeros(256).unsqueeze(0).expand(ys.size(1), -1).to(hs.device)
                    phone_intensity_rep = mean_rep.unsqueeze(1).expand(-1, hs.size(1), -1).squeeze(0)
                    # rep = rep.transpose(0,1)
                    # intensity_rep = phone_intensity_rep
                else:
                    mean_rep, rep = self.intensity_model.intensity_extractor(
                        intensity_input[i:i + 1],
                        emotion.view(-1).unsqueeze(-1)
                    )
                    # _, reduce_intensity = self.intensity_model.intensity_extractor(
                    #     intensity_input[i][:ds[i].sum() + 1].unsqueeze(0),
                    #     emotion.view(-1).unsqueeze(-1)
                    # )
                    # reduce_intensity = self.intensity_projector(rep.transpose(1,2))
                    # print(torch.sum(self.intensity_model.intensity_extractor.transformer.layers[0].linear1.weight))
                    # if rep.size() == 2:
                    #     reduce_intensity = rep.squeeze(0)
                    phone_intensity_rep = average_by_duration(rep.squeeze(0), ds[i])
                    # phone_intensity_rep = self.intensity_projector(phone_intensity_rep)
                frame_intensity_rep.append(rep.squeeze(0))
                intensity_rep.append(phone_intensity_rep)
                # mean_intensity_rep.append(mean_rep)
            frame_intensity_rep = torch.stack(frame_intensity_rep)
            frame_intensity_reps = self.intensity_projector_frame(frame_intensity_rep)
            phone_intensity_rep = pad_sequences(intensity_rep)
            phone_intensity_reps = self.intensity_projector_phone(phone_intensity_rep).squeeze(0)
            condition = torch.torch.cat([frame_intensity_reps, sid_embs.expand(-1, ys.size(1), -1)], dim=-1)

        # condition = None
        # mean_reps = torch.stack(mean_intensity_rep)
        # padded_intensity_rep = padded_intensity_rep.squeeze(1)

        # LayerNorm & Scale 적용
        # regurarized_intensity_rep = self.intensity_scale * self.intensity_norm(intensity_rep)
        # condition = torch.cat([sid_embs, reduce_intensity], dim=-1)
        # condition = sid_embs + intensity_rep
        # pre_intensity = expand_intensity.unsqueeze(1)
        sid_embs = sid_embs.expand(-1, hs.size(1), -1)
        # intensity_rep = intensity_rep.expand(-1, hs.size(1), -1)

        # # # regurarized_intensity_rep = regurarized_intensity_rep.unsqueeze(1).expand(-1, hs.size(1), -1)
        # # # Expand & Concat
        # emo_embs = emo_embs.unsqueeze(1)
        # condition = torch.cat([sid_embs, emo_embs], dim=-1)
        # # condition = torch.cat([intensity_rep, sid_embs], dim=-1)
        #
        # # condition = sid_embs + emo_embs
        # # # #
        # # condition = condition.unsqueeze(1)
        # spk_emo = condition.expand(-1, hs.size(1), -1)
        # hs = hs + condition
        # hs = torch.torch.cat([hs, spk_emo], dim=-1)
        if phone_intensity_reps.size(1) != sid_embs.size(1):
            print('wrong_size')
            blank_size = phone_intensity_reps.size(1) - sid_embs.size(1)
            phone_intensity_repses = torch.zeros((phone_intensity_reps.size(0), sid_embs.size(1), phone_intensity_reps.size(2))).to(hs.device)
            phone_intensity_repses[:,:blank_size,:] = phone_intensity_reps
        else:
            phone_intensity_repses =phone_intensity_reps
        spk_it = torch.cat([phone_intensity_repses, sid_embs], dim=-1)
        hs = torch.cat([hs, spk_it], dim=-1)
        hs = self.hs_projector(hs)
        #
        ...
        if self.stop_gradient_from_pitch_predictor:
            ...
            if self.use_pitch_max:
                p_m_outs = self.pitch_max_predictor(hs, d_masks.unsqueeze(-1), styles)
        ...
        if is_inference:
            ...
            hs, _ = self.length_regulator(hs, ds, alpha, olens.max())  # (B, T_feats, adim)
        ...
        if self.use_conditional_normalize:
            ...
            # zs_intensity = pre_intensity.expand(-1, zs.size(1), -1)
            # zs = zs + zs_intensity
        ...
        # os.makedirs('./train_weired_JCU', exist_ok=True)
        # # 첫번째 샘플(batch[0])의 실제 길이만큼만 잘라서 저장
        # actual_length = olens[0].item()  # 첫번째 샘플의 실제 길이
        # valid_sample = after_outs[0, :actual_length, :]  # [actual_length, odim]
        # np.save(f'./train_weired_JCU/{emotions[0]}_{sids[0]}', valid_sample.detach().cpu().numpy())
        if self.use_cwt and self.use_classifier and self.use_pitch_max:
            ...
            return before_outs, after_outs, d_outs, p_outs, e_outs, p_m_outs, condition
        # elif self.use_equalizer:
    ...
    def inference(
        ...
        if self.use_cwt and self.use_classifier and self.use_pitch_max:
            ...
            if use_teacher_forcing:
                ...
                _, outs, d_outs, p_outs, e_outs, _, _ = self._forward(
                    xs,
                    ilens,
                    ys,
                    olens=olens,
