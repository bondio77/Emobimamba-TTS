...
from tts.discriminator.loss import AdversarialLoss
...
def disc_configure_optimizers(disc):
    ...
    scheduler_disc = {
        "scheduler": _disc_scheduler(optimizer_disc),
        "interval": "step",
        "frequency": 1,
    }

    return optimizer_disc, scheduler_disc
...
def model_param_setting(model_config, preprocess_config):
    ...
    with open("/home/lee/Desktop/INSUNG/intensity_fs2/ESD_preprocess_vocs_1/symbols.json",
              'r') as symbol_file:
...
def add_module_prefix(state_dict):
    """상태 딕셔너리에 'module.' 접두사를 추가하는 함수"""
    new_state_dict = {}
    for key, value in state_dict.items():
        new_key = f'module.{key}'  # 'module.' 접두사 추가
...
def resume(
    ...
    if train:
        ...
        disc_optimizers, disc_schedulers = disc_configure_optimizers(discriminator)
        disc_optimizers.load_state_dict(states['disc_optimizers'])
        disc_schedulers.update(states['disc_schedulers'])

        with open('/home/lee/Desktop/INSUNG/intensity_fs2/tts_tool/tts/fastspeech2/intensity.yaml', 'r') as f:
            config = yaml.safe_load(f)
        model.intensity_model = RankModel(config).to('cpu').eval()
        ...
        return model, optimizers, schedulers, discriminator, adversarial_loss, disc_optimizers, disc_schedulers
    ...
    # no_finetune
    # if not list(states.keys())[0].startswith('module.'):
    #     model_state = add_module_prefix(states)
    #
    # if reporter is not None and "reporter" in model_state:
    #     reporter.load_state_dict(model_state["reporter"])
    # else:
...
def get_model(configs, args=None, reporter=None, device=None, train=False):
    ...
    if args.resume:
        if train:
            ...
            model, optimizers, schedulers, discriminator, adversarial_loss, disc_optimizers, disc_schedulers = resume(
    ...
    if train:
        ...
        model.train()
        return model, optimizers, schedulers, discriminator, adversarial_loss
